...
function resolveLogoutRedirectUrl(req, cb) {
  ...
  passportUtils.getAuthConfig(req, req.authenticationType, function (err, authConfig) {
    ...
    const redirectUrl = urlUtils.join(authConfig.url, path);
    if (!logoutPage) {
    ...
    if (req.authenticationType === 'ias') {
      ...
      if (process.env.DISABLE_XSUAA_ENCODING === 'true') { // don't encode for backward compatability
        cb(null, redirectUrl + '?redirect=' + urlUtils.join(appRouterUrl, logoutPage) + '&client_id=' + authConfig.clientid);
      } else {
        logoutPageUrl = encodeURIComponent(urlUtils.join(appRouterUrl, logoutPage));
        cb(null, redirectUrl + '?redirect=' + logoutPageUrl + '&client_id=' + authConfig.clientid);
      }
    }
  });
}